Sub XLCode()
    ...
    Dim sourceType As String, startPlanVersion, sheetNames As Variant, sheetName As Variant, valueField As String
    ...
    For Each sheetName In sheetNames
        ...
        If sheetName = "Warehousing costs per kg" Then
            valueField = "Warehouse"
        ElseIf sheetName = "Distribution costs per kg" Then
            valueField = "Shipping"
        End If
        ...
        With rngInput
            For row = 1 To rngInput.Rows.Count
                If Not IsEmpty(.Cells(row, 1)) Then
                    For period = startPeriod To endPeriod
                        If .Cells(row, period + 3) <> 0 Then
                            ...
                            rs.Fields("SourceType") = valueField
                            ...
                            rs.Fields(valueField) = -1 * IIf(IsError(.Cells(row, period + 3)), 0, .Cells(row, period + 3))
                        End If
                    Next period
                End If
            Next row
    ...
    connection.Execute "DELETE FROM tblFacts WHERE SourceType = 'Warehouse' AND PlanVersion = " & Quot(planVersion) & " AND Country = " & _
        Quot(country) & " AND Period BETWEEN " & Quot(periodFrom) & " AND " & Quot(periodTo)
    connection.Execute "DELETE FROM tblFacts WHERE SourceType = 'Shipping' AND PlanVersion = " & Quot(planVersion) & " AND Country = " & _
        Quot(country) & " AND Period BETWEEN " & Quot(periodFrom) & " AND " & Quot(periodTo)
